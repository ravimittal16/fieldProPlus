      ...
      function(
          ...
          if (saveFields === true) {
            ...
            if (angular.isArray(equipCts) && equipCts.length > 0) {
              ...
              if (vm.view.currentEquip) {
                equipmentFactory
                  .saveDataCustomTypes({
                    num: vm.view.currentEquip.num,
                    barcode: vm.barcode,
                    customTypesJson: equipCtsJson
                  })
                  .then(function() {
        ...
        function getEquipmentsByBarcode(refreshArrayIndex, saveFields) {
          ...
          fpmUtilitiesFactory.showLoading().then(function() {
            equipmentFactory
              .getBarcodeEquipments(vm.barcode)
        ...
        function deleteEquipmentClicked() {
          if (vm.view.currentEquip) {
            ...
            alerts.confirm("Confirmation!", "Are you sure?", function() {
              fpmUtilitiesFactory.showLoading().then(function() {
                equipmentFactory
                  .unattachEquipToBarcode(vm.barcode, equip.num)
                  .then(function(response) {
                    if (response === true) {
                      alerts.alert(
                        "Success",
                        "Equipment unattached successfully",
                        function() {
        ...
        function showMaintenanceHistory() {
          ...
          if (vm.view.currentEquip) {
            if (vm.view.equipmentHistoryModal === null) {
              fpmUtilitiesFactory
                .getModal("equipmentHistoryModal.html", $scope)
                .then(function(modal) {
            ...
            fpmUtilitiesFactory.showLoading().then(function() {
              equipmentFactory
                .getEquipmentHistory(vm.view.currentEquip.num)
                .then(function(response) {
                  if (response) {
                    vm.result = response;
        ...
        var newEquipemtSchama = {
          num: 0,
          displayName: vm.displayName,
          numFromCustomerFile: 0,
          equipmentNumber: "",
          description: "",
          uin: vm.uin,
          customTypesJson: ""
        ...
        function createNewEquipment() {
          ...
          if (vm.view.newEquipmentModal === null) {
            fpmUtilitiesFactory
              .getModal("newEquipmentModal.html", $scope)
              .then(function(modal) {
        ...
        vm.events = {
          ...
          createEquipmentSubmit: function(isValid, saveAndAttach) {
            ...
            if (vm.view.newEquipmentEntity.num === 0) {
              vm.view.newEquipmentEntity.customTypesJson = JSON.stringify(
                defaultCustomTypes
              );
            }
            fpmUtilitiesFactory
              .showLoading("creating equipment..")
              .then(function() {
                equipmentFactory
                  .createNewEquipment(
                    vm.uin,
                    vm.view.newEquipmentEntity,
                    vm.barcode,
                    saveAndAttach
                  )
                  .then(function(response) {
                    if (response != null && angular.isArray(response)) {
                      ...
                      var msg = saveAndAttach
                        ? "Equipment saved and attached successfully"
                        : "Equipment " +
          ...
          onAttachEquipmentItemClicked: function(equip) {
            ...
            fpmUtilitiesFactory.showLoading().then(function() {
              equipmentFactory
                .attachEquipToBarcode(vm.barcode, equip.num)
                .then(function(response) {
                  if (response !== null && angular.isArray(response)) {
                    ...
                    alerts.alert(
                      "Success",
                      "Equipment attached to work order successfully.",
                      function() {
