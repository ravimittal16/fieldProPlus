    ...
    function initController(
        ...
        vm.timePicker = {
            ...
            onTimeNewPicked: function () {
                ...
                var __isClockout =
                    _timeObj.jobCode === jobCodes.CLOCK_IN &&
                    vm.timePicker.prop === "finishTime";
                ...
                if (__isClockout) {
                    ...
                    _e.startTime = kendo.toString(
                        kendo.parseDate(_timeObj.startTime),
                        "g"
                    );
                    _e.finishTime = kendo.toString(
                        kendo.parseDate(_timeObj.finishTime),
                        "g"
                    );
                        ...
                        timecardFactory
                            ...
                            .then(function (response) {
                                if (response.errors === null) {
                                    ...
                                    if (
                                        vm.ui.data.timeCards.length !==
                                        response.timeCardDetails.length
                                    ) {
                                        vm.ui.data.approvalStatus =
                                            response.timeCardSummary
                                                .approveStatus || 0;
                                    }
                                    $timeout(function () {
                                        _updateTimeCardsArray(
            ...
            onTimeClicked: function (t, isForStartTime) {
                ...
                if (
                    __status === statusTypes.SEND_FOR_APPROVAL ||
                    __status === statusTypes.APPROVED ||
                    __status === statusTypes.RESENT_FOR_APPROVAL
                ) {
                    return false;
                }
                var __isClockout =
                    t.jobCode === jobCodes.CLOCK_IN &&
                    !isForStartTime &&
                    t.finishTime === null;
                if (__isClockout) {
                    var notCheckInDetails = _.filter(
                        vm.ui.data.timeCards,
                        function (tc) {
                            return (
                                tc.finishTime === null &&
                                tc.jobCode !== jobCodes.CLOCK_IN
                            );
                        }
                    );
        ...
        function __showDateTimePicker(t, isForStartTime) {
            ...
            if (t[vm.timePicker.prop]) {
                var __dateText = kendo.toString(
                    kendo.parseDate(t[vm.timePicker.prop]),
                    "g"
                );
        ...
        function _updateTimeCardBindings(details) {
            vm.ui.data.timeCards = [];
            vm.ui.data.checkInOuts = _.filter(
            ...
            vm.ui.data.approvalStatus =
                details.timeCardSummary.approveStatus || 0;
            ...
            if (angular.isDefined(vm.ui.data.currentClockedIn)) {
                ...
                vm.ui.data.clockOutDateTime = kendo.parseDate(
                    clockIn.finishTime
                );
        ...
        function _calculateTotalPayableTime() {
            ...
            var payables = _.filter(vm.ui.data.timeCards, function (tc) {
                return (
                    tc.jobCode === jobCodes.CLOCK_IN && tc.finishTime !== null
                );
            });
            ...
            if (checkins.length > 0) {
                ...
                if (
                    distinctClockInNums.length == 1 &&
                    distinctClockInNums[0] == null
                )
                    vm.hideTotalRow = true;
                angular.forEach(distinctClockInNums, function (cn, i) {
                    var totalCheckinsMins = 0;
                    angular.forEach(checkins, function (e, i) {
                        if (e.clockInNum == cn)
                            totalCheckinsMins += moment(
                                kendo.parseDate(e.finishTime)
                            ).diff(kendo.parseDate(e.startTime), "minutes");
                    });
                    var hours = Math.floor(totalCheckinsMins / 60);
                    var mintues = totalCheckinsMins % 60;
                    vm.ui.data.totalCheckinTime[cn] =
            ...
            if (nonPayables.length > 0) {
                ...
                angular.forEach(nonPayables, function (e, i) {
                    totalNonPMins += moment(kendo.parseDate(e.finishTime)).diff(
                        kendo.parseDate(e.startTime),
                        "minutes"
                    );
        ...
        function _getTimeCardByDate() {
            $timeout(function () {
                ...
                fpmUtilitiesFactory.showLoading().then(function () {
                    timecardFactory
                        .getTimeCardByDate(toDateString(vm.currentDate))
                        .then(function (response) {
                            if (response) {
        ...
        $ionicPopover
            .fromTemplateUrl("my-popover.html", {
                scope: $scope
            })
        ...
        vm.isPtoClockIn = function (tc) {
            ...
            var _hasLinkedWithPto = _.filter(
                vm.ui.data.timeCards,
                function (t) {
                    return (
                        t.ptoClockInLinkedWith &&
                        t.ptoClockInLinkedWith === tc.num
                    );
                }
            );
        ...
        function _processClockOutUser(clockInDateTime, __details) {
            ...
            if (vm.ui.data.currentClockedIn) {
                details["uniqueIdentifier"] =
                    vm.ui.data.currentClockedIn.uniqueIdentifier;
            } else {
                ...
                timecardFactory
                    ...
                    .then(function (response) {
                        if (response && response.errors === null) {
                            ...
                            vm.ui.data.approvalStatus =
                                response.timeCardSummary.approveStatus || 0;
        ...
        function showModal() {
            ...
            if (vm.ui.data.addEditDetailsModal === null) {
                $ionicModal
                    .fromTemplateUrl("timecardDetailsModal.html", {
                        scope: $scope,
                    ...
                    .then(function (modal) {
                        ...
                        $timeout(function () {
                            $scope.$broadcast(
                                "timecard:addEditDetailsModal:open",
                                {
        ...
        function showSummaryModal() {
            if (vm.ui.data.timeCardSummaryModal === null) {
                $ionicModal
                    .fromTemplateUrl("timecardSummaryModal.html", {
                        scope: $scope,
                        animation: "slide-in-up"
                    })
                    .then(function (modal) {
        ...
        function showTutorialModal() {
            if (vm.ui.data.timecardTutorialModal === null) {
                $ionicModal
                    .fromTemplateUrl("timecardTutorialModal.html", {
                        scope: $scope,
                        animation: "slide-in-up"
                    })
                    .then(function (modal) {
        ...
        function _processClearOrDeleteClockOut(
            numFromDetail,
            fromDeleteClockIn,
            callback
        ) {
            alerts.confirm(
                ...
                function () {
                    ...
                    fpmUtilitiesFactory.showLoading().then(function () {
                        timecardFactory
                            .clearClockOutTime(numFromDetail, fromDeleteClockIn)
                            .then(function (response) {
                                if (response) {
                                    $timeout(function () {
                                        _getTimeCardByDate();
        ...
        vm.ui = {
            ...
            events: {
                ...
                onClockInOutActionClicked: function (detail) {
                    var hideSheet = $ionicActionSheet.show({
                        ...
                        destructiveButtonClicked: function () {
                            ...
                            // ==========================================================
                            var _pendingClockout = vm.ui.data.timeCards.filter(
                                function (t) {
                                    return (
                                        t.jobCode === jobCodes.CLOCK_IN &&
                                        t.finishTime === null
                            ...
                            if (_pendingClockout.length > 0) {
                                ...
                                alerts.alert(
                                    "Warning",
                                    "You cannot have multiple Clock Out."
                                );
                                return false;
                            }
                            _processClearOrDeleteClockOut(
                    ...
                    alerts.confirm(
                        ...
                        function () {
                            ...
                            if (_ft.isBefore(_st)) {
                                alerts.alert(
                                    "Invalid Time",
                                    "Please enter finish time manually.",
                                    function () {
                                        $timeout(function () {
                ...
                onAddScheduleCompleted: function (o) {
                    if (
                        vm.ui.data.timeCards.length !== o.timeCardDetails.length
                    ) {
                        vm.ui.data.approvalStatus =
                ...
                onCardActionClicked: function (t) {
                    $ionicActionSheet.show({
                        ...
                        destructiveButtonClicked: function () {
                            if (
                                !t.isUserDefined ||
                                vm.timecardAccessLevel == 1
                            ) {
                                alerts.alert(
                                    "Invalid Action",
                                    "You are not allowed to perform delete"
                                );
                            } else {
                                            ...
                                            timecardFactory
                                                ...
                                                .then(function (response) {
                                                    if (response) {
                                                        alerts.alert(
                                                            "Success",
                                                            "Time Details cleared successfully",
                                                            function () {
                                                                _updateTimeCardsArray(
                                                                    response.timeCardDetails
                                                                );
                ...
                sendForApproval: function (status) {
                    if (
                        angular.isArray(vm.ui.data.timeCards) &&
                        vm.ui.data.timeCards.length === 0
                    ) {
                        alerts.alert(
                            "No Timecard!",
                            "No Timecard information found to process this request."
                        );
                        return false;
                    }
                    alerts.confirm(
                                ...
                                timecardFactory
                                    ...
                                    .then(function (response) {
                                        if (response) {
                                            ...
                                            alerts.alert(
                                                "Success",
                                                "Time card sent for approval successfully"
                                            );
                                            vm.ui.data.addTimeVisibility = false;
                                            vm.ui.data.ptoButtonVisibility = false;
                                            vm.ui.data.approvalStatus =
                                                vm.ui.data.summary.approveStatus;
                                        }
                                    })
                                    .finally(fpmUtilitiesFactory.hideLoading);
                            });
                ...
                clockInClick: function () {
                    ...
                    var details = {
                        startTime:
                            fpmUtilitiesFactory.toStringDate(clockInTime),
                        jobCode: jobCodes.CLOCK_IN,
                        numFromSummary:
                            vm.ui.data.summary === null
                                ? 0
                                : vm.ui.data.summary.Num,
                        timeCardDate:
                            fpmUtilitiesFactory.toStringDate(timeCardDate)
                    };
                    vm.errors = [];
                    fpmUtilitiesFactory.showLoading().then(function () {
                        timecardFactory
        ...
        function activateController() {
            ...
            vm.ui.data.enableForCustomer =
                angular.isDefined(havingCustomrNumber);
