    ...
    function __detailController(
        ...
        vm.timePicker = {
            ...
            onTimeNewPicked: function () {
                ...
                var __isClockout =
                    _timeObj.jobCode === __jobCodes.CLOCK_IN &&
                        ...
                        timecardFactory
                            ...
                            .then(function (response) {
                                if (response.errors === null) {
                                    ...
                                    if (
                                        vm.data.timeCards.length !==
                                        response.timeCardDetails.length
                                    ) {
                                        vm.data.approvalStatus =
                                            response.timeCardSummary
                                                .approveStatus || 0;
                                    }
                                    $timeout(function () {
                                        __updateTimeCardsArray(
            ...
            onTimeClicked: function (t, isForStartTime) {
                ...
                var __isClockout =
                    t.jobCode === __jobCodes.CLOCK_IN &&
                    !isForStartTime &&
                    t.finishTime === null;
                if (
                    __status === __statusTypes.SEND_FOR_APPROVAL ||
                    __status === __statusTypes.APPROVED ||
                    __status === __statusTypes.RESENT_FOR_APPROVAL ||
                    (!t.isUserDefined && t.jobCode !== __jobCodes.CLOCK_IN)
                ) {
                    return false;
                }
                if (__isClockout) {
                    var __notCheckInDetails = _.filter(
                        vm.data.timeCards,
                        function (tc) {
                            return (
                                tc.finishTime === null &&
                                tc.jobCode !== __jobCodes.CLOCK_IN
                            );
                        }
                    );
        ...
        function __calculateTotalPayableTime() {
            ...
            var payables = _.filter(vm.data.timeCards, function (tc) {
                return (
                    tc.jobCode === __jobCodes.CLOCK_IN && tc.finishTime !== null
                );
            });
        ...
        vm.data = {
            ...
            events: {
                sendForApproval: function (status) {
                    if (
                        angular.isArray(vm.data.timeCards) &&
                ...
                onAddScheduleCompleted: function (__res) {
                    if (
                        vm.data.timeCards.length !==
                        __res.timeCardDetails.length
                    ) {
                        vm.data.approvalStatus =
                            __res.timeCardSummary.approveStatus || 0;
                    }
                ...
                clockInClick: function () {
                    ...
                    var details = {
                        ...
                        numFromSummary: vm.data.summary
                            ? vm.data.summary.num
                            : 0,
        ...
        function __updateBindingsForSummaryStatus(details) {
            ...
            if (
                anythingPending.length > 0 ||
                vm.data.approvalStatus !== __statusTypes.NONE
            ) {
        ...
        function __updateTimeCardBindings(details) {
            ...
            vm.data.checkInOuts = _.filter(
                details.timeCardDetails,
                function (e) {
                    return (
                        e.jobCode !== __jobCodes.CLOCK_IN &&
                        e.jobCode !== __jobCodes.CLOCK_OUT
                    );
                }
            );
        ...
        $ionicPopover
            .fromTemplateUrl("timecard-popover.html", {
                scope: $scope
            })
