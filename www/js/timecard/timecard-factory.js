(function () {
    ...
    function initFactory(apicontext, $rootScope, $q, $cacheFactory) {
        ...
        function clearTimeCard(summaryId) {
            return apicontext.get(
                baseUrl + "ClearTimeCard?summaryId=" + summaryId
            );
        }
        ...
        function getSummaryPayableHours(filters) {
            return apicontext.post(
                baseUrl + "GetPayableHoursSummaryView",
                filters
            );
        }
        ...
        function verifyKioskPin(pin) {
            return apicontext.get(baseUrl + "VerifyKioskPin?pin=" + pin);
        }

        function kioskClockInOut(files, model, userNum, isForClockIn) {
            var _p = _attachLocationCoordinates(model);
            return apicontext.upload(
                baseUrl +
                    "KioskClockInOutUser?userNum=" +
                    userNum +
                    "&isForClockIn=" +
                    isForClockIn,
                files,
        ...
        function updateUserKioskCache(timecardDetails, kioskUserNum) {
            var kioskCache = cache.get("$timecard.kiosk");
            if (angular.isDefined(kioskCache) && kioskCache) {
                kioskCache[kioskUserNum] = timecardDetails;
                cache.put("$timecard.kiosk", kioskCache);
            } else {
                var __kioskCach = {};
                __kioskCach[kioskUserNum] = timecardDetails;
                cache.put("$timecard.kiosk", __kioskCach);
            }
        }

        function checkifKioskEnabled(includeUserCheck) {
            return apicontext.get(
                baseUrl +
                    "CheckifKioskEnabled?includeUserCheck=" +
                    includeUserCheck
            );
        }

        function allowExitKiosk(userKey) {
            return apicontext.get(
                baseUrl +
                    "AllowExitKiosk?userKey=" +
         ...
         * KIOSK FUNCTIONS
         */
        factory.updateUserKioskCache = updateUserKioskCache;
        factory.kioskClockInOut = kioskClockInOut;
        factory.verifyKioskPin = verifyKioskPin;
        factory.checkifKioskEnabled = checkifKioskEnabled;
        factory.allowExitKiosk = allowExitKiosk;
        /**
         * ================================================================================
         */
