    ...
    function initController(
        ...
        function extractJsonOrdersToLocalArray() {
            ...
            if (!vm.isServiceProvider || vm.havingGroupsAssigned) {
                angular.forEach(vm.result, function (r, i) {
                    if (
                        r.dataForAdministrator &&
                        r.dataForAdministrator.length > 0
                    ) {
                        _.forEach(r.dataForAdministrator, function (p) {
                            if (p.ordersJson) {
                                _.forEach(
                                    JSON.parse(p.ordersJson),
                                    function (o) {
                                        orders.push(o);
                                    }
                                );
                ...
                angular.forEach(vm.result, function (r, i) {
                    if (
                        r.dataForServiceProvider &&
                        r.dataForServiceProvider.length > 0
                    ) {
        ...
        vm.events = {
            ...
            inRouteClicked: function (odr) {
                if (timeCardInfo.enabled === false) {
                    ...
                    var runningClockIn = _.findWhere(
                        timeCardInfo.todaysClockIns,
                        {
                            finishTime: null
                        }
                    );
                ...
                if (orders && orders.length > 0) {
                    ...
                    var matchedOrders = _.filter(orders, function (o) {
                        return (
                            ...
                            o[displayNamePropName]
                                .toLowerCase()
                                .indexOf(tolower) > -1
                        );
                    });
                    vm.matchedOrders = _.uniq(
                        matchedOrders,
                        function (item, key, a) {
            ...
            onChildGroupClicked: function (item, type, prop) {
                ...
                if (item.isOpen === true && item.clickCount === 0) {
                    ...
                    if (type === "SP") {
                        item.dataForServiceProvider = JSON.parse(
                            item.ordersJson
                        );
                    } else {
        ...
        function _getTodaysTimeCardEntries() {
            ...
            timecardFactory.getTimeCardByDate(dt).then(function (response) {
                if (response) {
                    timeCardInfo.todaysClockIns = _.where(
                        response.timeCardDetails,
                        {
                            jobCode: jobCodes.CLOCK_IN
                        }
                    );
                    timeCardInfo.currentDetails = _.reject(
                        response.timeCardDetails,
                        {
                            jobCode: jobCodes.CLOCK_IN
                        }
        ...
        function activateController() {
            ...
            sharedDataFactory.updateLocationServiceStatus(
                vm.userInfo.userEmail
            );
            if (timeCardInfo.enabled === true) {
                _getTodaysTimeCardEntries();
            }
